<template>
  <section class="p20">
    ...
    <el-form ref="ruleForm2" :model="ruleForm2" label-width="100px" class="demo-ruleForm pb50 mt20" :inline="true">
      ...
      <el-form-item label="分类" prop="fclass" :rules="[{ required: true, message: '分类名称不能为空'},]">
        <el-select v-model="ruleForm2.fclass" placeholder="请选择">
          <el-option v-for="item in data.fclass" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
    ...
    <!-- 左边开始 -->
    <div class="flex mt20">
      <div class="tabs-box" style="overflow: auto; max-height: 644px;">
        <h3>基础组件</h3>
            ...
            <div class="ivu-tabs-tabpane">
              <div class="coms-list">
                <div id="modular">
                  ...
                  <div class="coms-item" @click="add('文本')"><i alt="" class="icon-temp  iconfont icon-title" />
                    <span> 单行文本</span></div>
                  <div class="coms-item" @click="add('多行文本')"><i alt="" class="icon-temp  iconfont icon-title" />
                    <span> 多行文本</span></div>
                  <div class="coms-item" @click="add('时间')"><i alt="" class="icon-temp  iconfont icon-title" />
                    <span> 时间</span></div>
                  <div class="coms-item" @click="add('日期')"><i alt="" class="icon-temp  iconfont icon-title" />
                    <span> 日期</span></div>
                  <div class="coms-item" @click="add('单项选择')"><i alt="" class="icon-temp  iconfont icon-title" />
                    <span> 单项选择</span></div>
                  <div class="coms-item" @click="add('多项选择')"><i alt="" class="icon-temp  iconfont icon-title" />
                    <span> 多项选择</span></div>
                  <div class="coms-item" @click="add('下拉框')"><i alt="" class="icon-temp  iconfont icon-title" />
                    <span> 下拉框</span></div>
                  <div class="coms-item" @click="add('上传图片')"><i alt="" class="icon-temp  iconfont icon-title" />
                    <span>上传图片</span></div>
